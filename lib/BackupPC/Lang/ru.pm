...
<table class="tableStnd tbl-Admin_Options_Page-reload">
  <tr><td>Перезагрузить настройки сервера:<td class="hasButtons"><input type="button" value="Перезагрузить"
...
<ul>
  <li><a href="\$MyURL\?action=editConfig">Редактирование Настроек</a></li>
  <li><a href="\$MyURL\?action=editConfig&newMenu=hosts">Редактирование Узлов</a></li>
</ul>
EOF
...
Введите каталог: <input type="text" name="dir" size="60" maxlength="4096" value="\${EscHTML(\$dir)}">
...
<input type="hidden" name="share" value="\${EscHTML(\$share)}">
<input type="hidden" name="action" value="browse">
Комментарий: <input type="text" name="comment" class="inputCompact" size="60" maxlength="4096" value="\${EscHTML(\$comment)}">
...
<li> Сопоставление имени общего ресурса с реальным клиентским путем (ClientShareName2Path):
    <ul>
\$share2pathStr
    </ul>
EOF
...
<li> Файлы из разных резервных копий, содержащие одно и то же,
     имеют один и тот же номер версии (PleaseTranslateThis: except between v3 and v4 backups);
